package jp.voicenotea.app.ui.screens
...
fun MemoListScreen(
    ...
    val fullText = viewModel.fullText.collectAsState()
    ...
    LaunchedEffect(successMessage.value) {
        successMessage.value?.let { message ->
            snackbarHostState.showSnackbar(message)
            // メッセージをクリアして、次回画面遷移時に重複表示されないようにする
    ...
    Scaffold(
        ...
        Column(
            ...
            if (isListening.value) {
                ListeningStatusBar(fullText.value)
            }
